...
int main(int argc, char **argv)
{
  ...
  srv.request.minDist = std::stoi(argv[2]);
  srv.request.maxDist = std::stoi(argv[3]);
  srv.request.numIterations = 20;

  /* read STL file and publish to rviz */
  std::vector<nav_msgs::Path> * mesh = readSTLfile(ros::package::getPath("request")+"/meshes/"+argv[1]);
