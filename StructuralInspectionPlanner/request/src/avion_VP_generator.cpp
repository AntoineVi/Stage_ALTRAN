...
std::vector<nav_msgs::Path> * readSTLfile(std::string name, double meshBounds[], double sF);
...
//  mesh = readSTLfile(ros::package::getPath("request")+"/meshes/asciiavion.stl", meshBounds, std::stof(argv[9]));
  mesh = readSTLfile(ros::package::getPath("request")+"/meshes/asciiavion.stl", meshBounds, 1.0);
  ...
  srv.request.spaceSize.push_back(1000.0);
  srv.request.spaceSize.push_back(1000.0);
  srv.request.spaceSize.push_back(1000.0);
  //srv.request.spaceSize.push_back(std::stof(argv[3]));
  //srv.request.spaceSize.push_back(std::stof(argv[4]));
  //srv.request.spaceSize.push_back(std::stof(argv[5]));

  ROS_INFO("Space area is bounded by: [%f,%f,%f]", srv.request.spaceSize[0], srv.request.spaceSize[1], srv.request.spaceSize[2]);
...
/*
  if(std::stof(argv[3])+std::stof(argv[4])+std::stof(argv[5])+std::stof(argv[6])+std::stof(argv[7])+std::stof(argv[8]) > 0) {
...
/*	  shape_msgs::SolidPrimitive body;
	  ...
	  body.dimensions.push_back(std::stof(argv[3]));
	  body.dimensions.push_back(std::stof(argv[4]));
	  body.dimensions.push_back(std::stof(argv[5]));
	  srv.request.obstacles.push_back(body);
	  geometry_msgs::Pose pose;
	  pose.position.x = std::stof(argv[6]);
	  pose.position.y = std::stof(argv[7]);
	  pose.position.z = std::stof(argv[8]);
	  pose.orientation.x = 0.0;
	  pose.orientation.y = 0.0;
	  pose.orientation.z = 0.0;
	  pose.orientation.w = 1.0;
	  srv.request.obstaclesPoses.push_back(pose);
	  srv.request.obstacleIntransparancy.push_back(std::stof(argv[10]));
	  ROS_INFO("obstacleIntransparancy:%d", srv.request.obstacleIntransparancy[0]);
...
std::vector<nav_msgs::Path> * readSTLfile(std::string name, double meshBounds[], double sF)
{
  ...
  while(0 != strcmp(strtok(line, " "), "endsolid") && !ros::isShuttingDown())
  {
    ...
    for(int i = 0; i<7; i++)
    {
      ...
      if(line[q] == 'v')
      {
        ...
        const double scaleFactor = sF;   // used to scale the mesh before processing
  ...
  meshBounds[0] = fabs(minX)+fabs(maxX);
  meshBounds[1] = fabs(minY)+fabs(maxY);
  meshBounds[2] = fabs(minZ)+fabs(maxZ);
  meshBounds[3] = maxZ; // define start pose
  return mesh;
}
