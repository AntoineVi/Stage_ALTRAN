...
template< typename T >
class Array2D : public OctreeArray<T,2>
{
public:
    Array2D();
    Array2D( int m, int n );
    Array2D( const OctreeArray<T,2>& array );
    ...
    : OctreeArray<T,2>()
{
    ...
    OctreeArray<T,2>::operator= ( OctreeArray<T,2>(sizes) );
}

template< typename T >
Array2D<T>::Array2D( const OctreeArray<T,2>& array )
    : OctreeArray<T,2>(array)
{
}
...
const T& Array2D<T>::at( int i, int j ) const
{
    ...
    indices(1) = i;
    indices(0) = j;
    return OctreeArray<T,2>::at(indices);
}
...
T& Array2D<T>::operator() ( int i, int j )
{
    ...
    indices(1) = i;
    indices(0) = j;
    return OctreeArray<T,2>::operator()(indices);
}
...
Array2D<T> Array2D<T>::subarray( int iBegin, int jBegin, int iEnd, int jEnd )
{
    ...
    return OctreeArray<T,2>::subarray( begin, end );
}
