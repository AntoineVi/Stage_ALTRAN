...
int octreeScaling(double value, double spaceSize);
...
int main(int argc, char **argv)
{
	...
	//sleep_time = 0.005;
	sleep_time = 0.0;
	octreeSize = 1024; // Because spaceSize = 1000 and octreeSzie needs to be 2^n
...
bool viewpointsGenerator(koptplanner::inspection::Request  &req, koptplanner::inspection::Response &res) {
	...
	for(int i = 0; i < maxID; i++)
	{
		...
		// Create and save an octree to optimise the configurations search
		Octree<int> octreeVP(octreeSize);
				
		for(int j=0; j<gVect.size(); j++) {			
			geometry_msgs::PoseStamped tmpVP;
			
			tmpVP.header.seq = i;
			tmpVP.header.stamp = ros::Time::now();
			tmpVP.header.frame_id = "/kopt_frame";
			
			tmpVP.pose.position.x = gVect[j][0];
			tmpVP.pose.position.y = gVect[j][1];
			tmpVP.pose.position.z = gVect[j][2];
#if DIMENSIONALITY>4
			tf::Quaternion q = tf::createQuaternionFromRPY(gVect[j][3],gVect[j][4],gVectSorted[j][5]);
#else
			tf::Quaternion q = tf::createQuaternionFromRPY(0.0,0.0,gVect[j][3]);
#endif
			tmpVP.pose.orientation.x = q.x();
			tmpVP.pose.orientation.y = q.y();
			tmpVP.pose.orientation.z = q.z();
			tmpVP.pose.orientation.w = q.w();
			
			octreeVP(octreeScaling(tmpVP.pose.position.x, req.spaceSize[0]), octreeScaling(tmpVP.pose.position.y, req.spaceSize[0]), octreeScaling(tmpVP.pose.position.z, req.spaceSize[0])) = 1;	
		}
		
		// Save the octree
		file.open((pkgPath+"/viewpoints/viewpoint_"+std::to_string(i)+".txt").c_str(), std::ios::out);
		octreeVP.writeBinary(file);
		delete &octreeVP;
	}    
...
int octreeScaling(double value, double spaceSize) {
	return (int) value*(octreeSize-1)/spaceSize+(octreeSize-1)/2.0;
}
...
std::vector<geometry_msgs::PoseStamped> readPath() {
	...
	if (file.is_open())	{
		while(getline(file,line)) {
			...
			tmpVP.header.seq = std::atoi(poseStr[0].c_str());
			tmpVP.header.stamp = ros::Time::now();
			tmpVP.header.frame_id = "/kopt_frame";
			
			tmpVP.pose.position.x = std::atof(poseStr[1].c_str());
			tmpVP.pose.position.y = std::atof(poseStr[2].c_str());
			tmpVP.pose.position.z = std::atof(poseStr[3].c_str());
			tmpVP.pose.orientation.x = std::atof(poseStr[4].c_str());
			tmpVP.pose.orientation.y = std::atof(poseStr[5].c_str());
			tmpVP.pose.orientation.z = std::atof(poseStr[6].c_str());
			tmpVP.pose.orientation.w = std::atof(poseStr[7].c_str());
			
			path.push_back(tmpVP);
		}
	}
	else {
