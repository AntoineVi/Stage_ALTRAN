...
std::fstream file;
double scaleVP;
std::vector<StateVector>* matrixVPs;

double euclideanDistance(geometry_msgs::PoseStamped pt1, StateVector pt2);
void saveConfigsAndMakeTourFile();
void publishViewpoint(StateVector stateVP, int VP_ind);

bool plan(koptplanner::inspection::Request	&req, koptplanner::inspection::Response &res);
...
bool plan(koptplanner::inspection::Request	&req, koptplanner::inspection::Response &res)
{
...
#endif
	matrixVPs = new std::vector<StateVector>[maxID];
	
	//Fill matrixVPs with the fixed startup point
	StateVector tmpPt;
	tf::Pose pose;
	tf::poseMsgToTF(*req.requiredPoses.begin(), pose);
	double yaw_angle = tf::getYaw(pose.getRotation());
	tmpPt[0] = req.requiredPoses.begin()->position.x;
	tmpPt[1] = req.requiredPoses.begin()->position.y;
	tmpPt[2] = req.requiredPoses.begin()->position.z;
#ifdef USE_ROTORCRAFT_MODEL
	tmpPt[3] = yaw_angle;
#elif defined USE_FIXEDWING_MODEL
	tmpPt[3] = 0.0;
	...
	tmpPt[6] = yaw_angle;
#endif
	matrixVPs[0].push_back(tmpPt);
	
	if(endPoint) {
		//Fill matrixVPs with the fixed end point
		tf::Pose pose;
		tf::poseMsgToTF(*req.requiredPoses.end(), pose);
		double yaw_angle = tf::getYaw(pose.getRotation());
		tmpPt[0] = req.requiredPoses.end()->position.x;
		tmpPt[1] = req.requiredPoses.end()->position.y;
		tmpPt[2] = req.requiredPoses.end()->position.z;
	#ifdef USE_ROTORCRAFT_MODEL
		tmpPt[3] = yaw_angle;
	#elif defined USE_FIXEDWING_MODEL
		tmpPt[3] = 0.0;
		...
		tmpPt[6] = yaw_angle;
	#endif
		matrixVPs[maxID-1].push_back(tmpPt);	
	}
	...
	while(!ros::isShuttingDown() && koptPlannerIteration < req.numIterations)
	{
		...
		for(int i = 0; i < maxID; i++)
		{
			...
			for(int k=0; k<gVect.size(); k++) {
				if(std::find(matrixVPs[i].begin(), matrixVPs[i].end(), gVect[k]) == matrixVPs[i].end()) {
					matrixVPs[i].push_back(gVect[k]);
					/* display sampled viewpoint in rviz */
					//publishViewpoint(gVect[k], i);	
				}
			}
			gVect.clear();			
		}
...
void saveConfigsAndMakeTourFile() {
	...
	fileTour.open((pkgPath+"/data/tourSAVE.txt").c_str(), std::ios::out | std::ios::trunc);
	if(!fileTour.is_open()) {
		ROS_ERROR("Could not open tour file");
		ros::shutdown();
	}
			...
			for(int i=0; i<maxID; i++) {
				for(std::vector<StateVector>::iterator itVP = matrixVPs[i].begin(); itVP != matrixVPs[i].end(); itVP++) {
					if( (*itVP)[0] == it->pose.position.x && (*itVP)[1] == it->pose.position.y && (*itVP)[2] == it->pose.position.z) {
						...
						fileTour << std::setprecision(8) << std::to_string((*itVP)[2])+"\t";
#if DIMENSIONALITY>4
						tf::Quaternion q = tf::createQuaternionFromRPY((*itVP)[3],(*itVP)[4],(*itVP)[5]);
#else
						tf::Quaternion q = tf::createQuaternionFromRPY(0.0,0.0,(*itVP)[3]);
#endif
						fileTour << std::setprecision(8) << q.x() << "\t";
						fileTour << std::setprecision(8) << q.y() << "\t";
						fileTour << std::setprecision(8) << q.z() << "\t";
						fileTour << std::setprecision(8) << q.w() << "\n";
						
						std::sort(matrixVPs[i].begin(), matrixVPs[i].end(), [it](StateVector pt1, StateVector pt2){ return euclideanDistance(*it, pt1) < euclideanDistance(*it, pt2); });
						...
						for(int j=0; j<matrixVPs[i].size(); j++) {
							file << std::setprecision(8) << std::to_string(matrixVPs[i][j][0])+"\t";
							file << std::setprecision(8) << std::to_string(matrixVPs[i][j][1])+"\t";
							file << std::setprecision(8) << std::to_string(matrixVPs[i][j][2])+"\t";
#if DIMENSIONALITY>4
							tf::Quaternion q = tf::createQuaternionFromRPY(matrixVPs[i][j][3],matrixVPs[i][j][4],matrixVPs[i][j][5]);
#else
							tf::Quaternion q = tf::createQuaternionFromRPY(0.0,0.0,matrixVPs[i][j][3]);
#endif
